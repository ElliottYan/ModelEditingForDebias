...
@torch.no_grad()
def compute_icl_bias_edit_quality(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    """
    sentence_pairs = record["original_pairs"]
    ...
    # ret = {}
    # ret["reverse_success_rate"], ret["lprobs"] = eval_reverse(model, tok, sentence_pairs, original_lprobs)

    # First, unpack rewrite evaluation record.
    target_new = record["target_new"]
    prompt = record["prompt"] + " "+ target_new
    # rephrase = record["rephrase_prompt"] if 'rephrase_prompt' in record.keys() else None
    new_fact = f'New Fact: {prompt}\nPrompt: '

    if pre_edit:
        success,lprobs = icl_lm_bias_eval(model, model_name, hparams, tok, icl_examples, sentence_pairs, original_lprobs, '')
    else:
        success, lprobs = icl_lm_bias_eval(model, model_name, hparams, tok, icl_examples, sentence_pairs, original_lprobs, new_fact)

    ret = {
        f"edited_lprobs": lprobs,
        "reverse_success_rate": success
    }
    ret['locality'] = {}

    icl_prefix = ''.join(icl_examples) + new_fact
    if 'locality' in record.keys() and any(record['locality']):
        # NOTE: in neighbourhood, we do not evaluate bias
        # thus, we use original icl_lm_eval here.
    ...
    # compute para lprobs
    if 'para_pairs' in record:
        # avoid OOM
        # torch.cuda.empty_cache()
        para_pairs = record['para_pairs']
        para_lprobs = eval_para(model, tok, para_pairs, device, prefix=icl_prefix)
        ret['para_lprobs'] = para_lprobs
 
    return ret

def icl_lm_bias_eval(
        model,
        model_name,
        hparams: HyperParams,
        tokenizer,
    ...
    prefix = ''.join(icl_examples) + x
    
    sent_lprobs_more = run_lprobs(model, tokenizer, sentence_pairs[0],device, prefix).item()
    sent_lprobs_less = run_lprobs(model, tokenizer, sentence_pairs[1],device, prefix).item()

    if (sent_lprobs_more-sent_lprobs_less)*(original_lprobs[0]-original_lprobs[1]) < 0:
        return 1, [sent_lprobs_more,sent_lprobs_less]
    else:
        return 0, [sent_lprobs_more,sent_lprobs_less]
...
def compute_bias_edit_quality(
    ...
    ret = {}
    ret["reverse_success_rate"], ret["lprobs"] = eval_reverse(model, tok, sentence_pairs, original_lprobs, device)
    ...
    # compute para lprobs
    if 'para_pairs' in record:
        para_lprobs = eval_para(model, tok, record['para_pairs'], device)
        ret['para_lprobs'] = para_lprobs
