...
def run_lprobs(model, tokenizer, sentence, device, prefix=None):
    ...
    # NOTE: truncation not applied? why sentence or new_sentence?
    target_ids = tokenizer(sentence, return_tensors='pt').to(device)
    inps = tokenizer(new_sentence, return_tensors='pt', max_length=2048, truncation=True).to(device)
